...
因此，MongoDB 的很多能力在这里并不能被充分利用，反而会带来额外的部署、运维和网络访问成本。对于一个主要承担“Redis 缓存后的持久化兜底”职责的存储层来说，这个方案偏重了。
...
Badger 同样是面向 KV 的嵌入式存储，而且是纯 Go 实现，与当前项目技术栈非常契合。对于我们的场景，它有几个明显优势：
...
Badger 也不是完全没有代价。比如批量写入过大时，需要处理 `badger.ErrTxnTooBig`；另外它并不擅长复杂查询，只适合当前这种基于主键读写的场景。
...
- MongoDB 更偏通用文档数据库，能力强，但对本场景来说偏重；
